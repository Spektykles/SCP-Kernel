				...
				const char *buf, size_t count)
{
	...
	int success = simple_strtoul(buf, 10, &oversampling);
	if (success == 0) {
		mutex_lock(&data->lock);
				...
				const char *buf, size_t count)
{
	...
	int success = simple_strtoul(buf, 10, &sw_oversampling);
	if (success == 0) {
		mutex_lock(&data->lock);
				...
				const char *buf, size_t count)
{
	struct bmp18x_data *data = dev_get_drvdata(dev);
	unsigned long delay;
	int success = simple_strtoul(buf, 10, &delay);
	if (success == 0) {
		mutex_lock(&data->lock);
		data->delay = delay;
		mutex_unlock(&data->lock);
				...
				const char *buf, size_t count)
{
	...
	int success = simple_strtoul(buf, 10, &enable);
	if (success == 0) {
		mutex_lock(&data->lock);
